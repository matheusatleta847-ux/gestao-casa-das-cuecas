...
# --- 1. DESIGN & CSS (LAYOUT VERTICAL ANTI-SOBREPOSIÇÃO) ---
...
st.markdown("""
    ...
    .metric-title { color: #64748B; font-size: 12px; font-weight: 600; text-transform: uppercase; }
    ...
    .primeiro-vez { border: 2px solid #22C55E; background: #F0FDF4; box-shadow: 0 4px 10px rgba(34, 197, 94, 0.15); }
...
# Filtro exclusivo para cálculo de conversão
...
k1.markdown(f'<div class="metric-card"><div class="metric-title">Faturamento (30d)</div><div class="metric-value">R$ {faturado:,.2f}</div></div>', unsafe_allow_html=True)
k2.markdown(f'<div class="metric-card"><div class="metric-title">Conversão Real</div><div class="metric-value">{taxa:.1f}%</div></div>', unsafe_allow_html=True)
k3.markdown(f'<div class="metric-card"><div class="metric-title">Atendimentos</div><div class="metric-value">{len(atendimentos_reais)}</div></div>', unsafe_allow_html=True)
            ...
            if st.session_state.get(f"m_p_{v['id']}", False):
                with st.expander("Motivo da Saída:", expanded=True):
                    ...
                    if st.button("Confirmar", key=f"ok_{v['id']}"):
                        ...
                        run_db("INSERT INTO historico (vendedor, evento, motivo, data) VALUES (?,?,?,?)", (v['nome'], "SAÍDA", mot, datetime.now().isoformat()))
                        st.session_state[f"m_p_{v['id']}"] = False
                        st.rerun()
        ...
        for _, v in fora.iterrows():
            with st.container():
                ...
                if st.button(f"Retornar: {v['nome']}", key=f"ret_{v['id']}"):
                    run_db("INSERT INTO historico (vendedor, evento, motivo, data) VALUES (?,?,?,?)", (v['nome'], "RETORNO", v['motivo_pausa'], datetime.now().isoformat()))
                    run_db("UPDATE usuarios SET status='Esperando', ordem=?, motivo_pausa=NULL WHERE id=?", (next_ordem(), v['id']))
                    st.rerun()
...
with t2:
    ...
    if not dados_raw.empty:
        # Exportação XLSX com todas as movimentações
        buffer = io.BytesIO()
        try:
            log_completo = dados_raw.sort_values(by='data', ascending=False)
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                log_completo.to_excel(writer, sheet_name='Geral', index=False)
            st.download_button("📊 Baixar Excel Completo (Vendas + Almoço)", data=buffer.getvalue(), 
                             file_name=f"Relatorio_Geral_{datetime.now().strftime('%d_%m')}.xlsx",
                             mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        except: st.error("Erro ao gerar Excel. Instale 'openpyxl'.")
    ...
    if st.session_state.user['is_admin']:
        ...
        with st.container(border=True):
            ...
            if st.button("Cadastrar Novo"):
                if n:
                    log = n.lower().replace(" ", ".")
                    # Verificação manual antes do insert
                    check = run_db("SELECT * FROM usuarios WHERE login = ?", (log,), is_select=True)
                    if not check.empty:
                        st.error(f"O login '{log}' já existe!")
                    else:
        ...
        for _, r in equipe.iterrows():
            ...
            c_inf.write(f"👤 **{r['nome']}**")
            if c_btn.button("Excluir", key=f"del_{r['id']}"):
