...
# --- 4. KPIs DE INTELIGÊNCIA (TROCA SEPARADA) ---
...
trocas = dados_raw[dados_raw['evento'] == 'Troca']

# Oportunidade = Venda + Perda (Troca é neutro)
...
k3.markdown(f'<div class="metric-card">P.A. (Itens)<br><b>{pa_medio:.2f}</b></div>', unsafe_allow_html=True)
k4.markdown(f'<div class="metric-card">Total Trocas<br><b>{len(trocas)}</b></div>', unsafe_allow_html=True)
    ...
    with c_atend:
        ...
        for _, v in atend.iterrows():
            with st.container(border=True):
                st.write(f"**{v['nome']}**")
                if st.button("Finalizar Atendimento", key=f"fin_{v['id']}"):
            ...
            if st.session_state.get(f"f_{v['id']}", False):
                with st.expander("Resultado:", expanded=True):
                    ...
                    # Lógica para Troca ou Não Convertido
                    perda_motivo = ""
                    if res == "Não convertido":
                        perda_motivo = st.selectbox("Motivo Perda:", ["Preço", "Tamanho", "Só olhando", "Falta Cor"], key=f"p_{v['id']}")
                    elif res == "Troca":
                        perda_motivo = "Troca de Mercadoria"
                    ...
                    if st.button("Gravar", key=f"g_{v['id']}"):
                        run_db("INSERT INTO historico (vendedor, evento, motivo, valor, itens, data) VALUES (?,?,?,?,?,?)", 
                               (v['nome'], res, perda_motivo, vlr, it, get_now_br().isoformat()))
...
with t2:
    ...
    if not dados_raw.empty:
        ...
        with c1:
            # Faturamento
            fig1 = px.bar(vendas_sucesso.groupby('vendedor')['valor'].sum().reset_index(), x='vendedor', y='valor', title="Faturamento por Vendedor", color_discrete_sequence=['#22C55E'])
            st.plotly_chart(fig1, use_container_width=True)
        with c2:
            # Volume por tipo (Diferenciando Troca)
            df_vol = dados_raw[dados_raw['evento'].isin(['Sucesso', 'Não convertido', 'Troca'])]
            fig2 = px.histogram(df_vol, x='vendedor', color='evento', barmode='group', title="Atendimentos por Tipo",
                               color_discrete_map={'Sucesso': '#22C55E', 'Não convertido': '#EF4444', 'Troca': '#3B82F6'})
        ...
        st.download_button("📥 Baixar Excel Completo", data=buffer.getvalue(), file_name=f"Relatorio_{get_now_br().strftime('%d_%m_%Y')}.xlsx")
