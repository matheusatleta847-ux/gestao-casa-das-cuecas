...
def metricas_card(df):
    ...
    m3.markdown(f'<div class="metric-card">P.A.<br><b>{pa:.2f}</b></div>', unsafe_allow_html=True)
        ...
        for _, v in atend.iterrows():
            ...
            if st.session_state[f_key]:
                with st.expander("📝 Detalhes", expanded=True):
                    ...
                    vlr_v = st.number_input("Valor:", min_value=0.0, key=f"v_{v['id']}") if res_at == "Sucesso" else 0.0
                    it_v = st.number_input("Itens:", min_value=1, step=1, key=f"i_{v['id']}") if res_at == "Sucesso" else 0
                    if st.button("SALVAR", key=f"sv_{v['id']}"):
                        run_db("INSERT INTO historico (vendedor, evento, motivo, valor, itens, data) VALUES (?,?,?,?,?,?)", (v['nome'], res_at, res_at, vlr_v, it_v, get_now_br().isoformat()))
...
with t2:
    ...
    if st.session_state.user['is_admin']:
        with st.expander("➕ LANÇAR VENDA MANUAL (RETROATIVA)"):
            ...
            vend_m = col_m1.selectbox("Vendedor:", run_db("SELECT nome FROM usuarios", is_select=True))
            data_m = col_m2.date_input("Data:", value=date.today())
            res_m = col_m3.selectbox("Resultado:", ["Sucesso", "Não convertido", "Troca"], key="manual_res")
            
            val_m = st.number_input("Valor R$:", min_value=0.0) if res_m == "Sucesso" else 0.0
...
with t3:
    if st.session_state.user['is_admin']:
        ...
        if st.button("Salvar Meta"): run_db("UPDATE config SET valor = ? WHERE chave = 'meta_loja'", (meta_n,)); st.rerun()
        st.divider()
        n_v = st.text_input("Vendedor:"); 
