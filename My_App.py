...
st.markdown("""
    ...
    .metric-card { border-top: 4px solid #3B82F6 !important; }
...
# --- 4. CÁLCULOS DE PERFORMANCE & META ---
...
falta_vender = max(meta_atual - faturamento_total, 0)

# --- 5. CABEÇALHO DE META (NOVO) ---
st.markdown(f"""
    <div class="meta-container">
        <h3 style='margin:0;'>🎯 Meta da Loja: R$ {meta_atual:,.2f}</h3>
        <p style='margin:5px 0;'>Vendido: <b>R$ {faturamento_total:,.2f}</b> | Falta: <b style='color:#EF4444;'>R$ {falta_vender:,.2f}</b></p>
    </div>
""", unsafe_allow_html=True)

# Barra de progresso visual
cor_barra = "green" if faturamento_total >= meta_atual else "#3B82F6"
st.progress(percentual_meta)
if faturamento_total >= meta_atual:
    st.balloons()
    st.success("🚀 PARABÉNS! A META FOI BATIDA!")

# KPIs secundários
k1, k2, k3 = st.columns(3)
conversao = (len(vendas_sucesso) / (len(dados_raw[dados_raw['evento'].isin(['Sucesso', 'Não convertido'])])) * 100) if not dados_raw.empty else 0
pa_medio = (vendas_sucesso['itens'].sum() / len(vendas_sucesso)) if not vendas_sucesso.empty else 0

k1.markdown(f'<div class="metric-card">Conversão Real<br><b>{conversao:.1f}%</b></div>', unsafe_allow_html=True)
k2.markdown(f'<div class="metric-card">P.A. (Peças)<br><b>{pa_medio:.2f}</b></div>', unsafe_allow_html=True)
k3.markdown(f'<div class="metric-card">Total Trocas<br><b>{len(dados_raw[dados_raw["evento"] == "Troca"])}</b></div>', unsafe_allow_html=True)

# --- 6. OPERAÇÃO ---
t1, t2, t3 = st.tabs(["📋 OPERAÇÃO", "📊 DESEMPENHO", "⚙️ CONFIG"])
    ...
    with c_fila:
        ...
        for i, v in esp.iterrows():
            ...
            if col1.button("Atender", key=f"at_{v['id']}", type="primary"):
                ...
                st.rerun()
            if col2.button("Pausa", key=f"p_{v['id']}"):
            ...
            if st.session_state.get(f"m_{v['id']}", False):
                with st.expander("Motivo:"):
                    ...
                    if st.button("Confirmar", key=f"conf_{v['id']}"):
                        ...
                        run_db("INSERT INTO historico (vendedor, evento, motivo, data) VALUES (?,?,?,?)", (v['nome'], "SAÍDA", m, get_now_br().isoformat()))
                ...
                with st.expander("Resultado:", expanded=True):
                    ...
                    if st.button("Gravar", key=f"gv_{v['id']}"):
                        run_db("INSERT INTO historico (vendedor, evento, motivo, valor, itens, data) VALUES (?,?,?,?,?,?)", 
                               (v['nome'], res, res, vlr, it, get_now_br().isoformat()))
...
with t2:
    if not dados_raw.empty:
        fig1 = px.bar(vendas_sucesso.groupby('vendedor')['valor'].sum().reset_index(), x='vendedor', y='valor', title="Vendas por Vendedor", template="seaborn")
        st.plotly_chart(fig1, use_container_width=True)
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            dados_raw.to_excel(writer, index=False)
        st.download_button("📥 Exportar Dados", data=buffer.getvalue(), file_name="Auditoria.xlsx")

with t3:
    if st.session_state.user['is_admin']:
        st.subheader("🎯 Configurar Meta da Loja")
        nova_meta = st.number_input("Valor da Meta (R$):", min_value=0.0, value=meta_atual)
        if st.button("Atualizar Meta"):
            run_db("UPDATE config SET valor = ? WHERE chave = 'meta_loja'", (nova_meta,))
            st.success("Meta atualizada!"); st.rerun()
        ...
        for _, r in equipe.iterrows():
            ...
            c1.write(f"**{r['nome']}**")
            if c2.button("Remover", key=f"del_{r['id']}"):
