            ...
            if st.session_state.get(f"modal_p_{v['id']}", False):
                with st.expander("Motivo da Saída:", expanded=True):
                    ...
                    if st.button("Confirmar", key=f"ok_{v['id']}"):
                        ...
                        run_db("INSERT INTO historico (vendedor, evento, motivo, data) VALUES (?,?,?,?)", (v['nome'], "Saída-Pausa", mot, datetime.now().isoformat()))
...
with t2:
    ...
    if not atendimentos_reais.empty:
        ...
        # EXPORTAÇÃO EXCEL XLSX (Motor Universal)
        buffer = io.BytesIO()
        try:
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                atendimentos_reais.to_excel(writer, sheet_name='RelatorioVendas', index=False)
            st.download_button("📊 Baixar Relatório Excel (.xlsx)", data=buffer.getvalue(), 
                             file_name=f"Vendas_CasaCuecas_{datetime.now().strftime('%d_%m')}.xlsx",
                             mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        except: st.error("Erro ao gerar Excel. Verifique se 'openpyxl' está instalado.")
