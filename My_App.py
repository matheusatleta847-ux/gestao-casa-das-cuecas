    ...
    with c_atend:
        ...
        for _, v in vendedores[vendedores['status'] == 'Atendendo'].iterrows():
            ...
            if st.session_state.get(f"fin_{v['id']}", False):
                with st.expander("Finalizar Atendimento AGORA", expanded=True):
...
with t2:
    st.subheader("📊 Filtro e Lançamentos Retroativos")
    ...
    if st.session_state.user['is_admin']:
        with st.expander("➕ LANÇAR VENDA PASSADA (Retroativa)"):
            ...
            if st.button("Gravar Venda Antiga"):
                ...
                st.success(f"Venda de {v_antigo} em {d_antiga} gravada!"); st.rerun()
    ...
    if not df_f.empty:
        ...
        c1.metric("Faturamento do Dia", f"R$ {vendas['valor'].sum():,.2f}")
