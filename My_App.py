...
# --- 4. TABS PRINCIPAIS ---
t1, t2, t3 = st.tabs(["📋 OPERAÇÃO", "📊 HISTÓRICO & EDIÇÃO", "⚙️ EQUIPE"])

with t1:
    meta_res = run_db("SELECT valor FROM config WHERE chave = 'meta_loja'", is_select=True)
    meta_val = meta_res.iloc[0,0] if not meta_res.empty else 5000.0
    ...
    with c2:
        ...
        for _, v in atend.iterrows():
            # Aqui usamos o Session State para o formulário não fechar
            form_key = f"form_aberto_{v['id']}"
            if form_key not in st.session_state: st.session_state[form_key] = False

            with st.container(border=True):
                st.write(f"Vendedor: **{v['nome']}**")
                if st.button("Finalizar Atendimento", key=f"btn_f_{v['id']}"):
                    st.session_state[form_key] = True # Ativa a memória de que a janela deve estar aberta

            if st.session_state[form_key]:
                with st.expander("📝 Detalhes do Fechamento", expanded=True):
                    dt_venda = st.date_input("Data da Venda:", value=date.today(), key=f"dt_{v['id']}")
                    ...
                    it_v = st.number_input("Itens:", min_value=0, step=1, key=f"i_{v['id']}")
                    
                    c_btn1, c_btn2 = st.columns(2)
                    if c_btn1.button("✅ SALVAR", key=f"save_{v['id']}", type="primary"):
                        data_f = datetime.combine(dt_venda, datetime.now().time()).isoformat()
                        run_db("INSERT INTO historico (vendedor, evento, motivo, valor, itens, data) VALUES (?,?,?,?,?,?)", 
                               (v['nome'], res_at, res_at, vlr_v, it_v, data_f))
                        run_db("UPDATE usuarios SET status='Esperando', ordem=? WHERE id=?", (get_next_ordem(), v['id']))
                        st.session_state[form_key] = False # Fecha a janela após salvar
                        st.rerun()
                    
                    if c_btn2.button("❌ CANCELAR", key=f"canc_{v['id']}"):
...
with t2:
    ...
    ver_tudo = st.checkbox("🔓 Ver todo o histórico")
    
    # Filtro de hoje por padrão
    hoje_filtro = date.today().strftime('%Y-%m-%d')
    query = "SELECT * FROM historico ORDER BY data DESC" if ver_tudo else f"SELECT * FROM historico WHERE data LIKE '{hoje_filtro}%' ORDER BY data DESC"
    df_h = run_db(query, is_select=True)

    if not df_h.empty:
        st.info("💡 Dica: Você pode editar valores diretamente na tabela e clicar em salvar abaixo.")
        edt_h = st.data_editor(df_h, hide_index=True, use_container_width=True)
        if st.button("💾 SALVAR ALTERAÇÕES"):
            for _, r in edt_h.iterrows():
        ...
        st.warning("Nenhum dado encontrado para hoje.")

with t3:
    if st.session_state.user['is_admin']:
        st.subheader("⚙️ Configurações Admin")
        m_ajuste = st.number_input("Meta Diária:", value=float(meta_val))
        if st.button("Salvar Nova Meta"):
            run_db("UPDATE config SET valor = ? WHERE chave = 'meta_loja'", (m_ajuste,))
            st.success("Meta atualizada!")
        
        st.divider()
        n_vendedor = st.text_input("Nome do Vendedor:")
        if st.button("Adicionar à Equipe"):
            if n_vendedor:
                run_db("INSERT OR IGNORE INTO usuarios (nome, login, status, ordem) VALUES (?,?,?,?)", 
                       (n_vendedor.title(), n_vendedor.lower().replace(" ","."), 'Fora', 0))
                st.rerun()
