...
st.markdown("""
    ...
    /* CARD UNIFICADO MONDAY - TUDO FICA DENTRO DESTA CAIXA */
    .monday-card-pro {
...
def init_db():
    # Adicionada coluna motivo_pausa para rastrear Almoço/Café etc
...
if st.session_state.pagina == "OPERAÇÃO":
        ...
        for _, v in vendedores[vendedores['status'] == 'Atendendo'].iterrows():
            ...
            if res == "Sucesso":
                ...
                if st.button("GRAVAR", key=f"ff_{v['id']}", type="primary", use_container_width=True):
                    run_db("INSERT INTO historico (vendedor, evento, motivo, valor, itens, data) VALUES (?,?,?,?,?,?)", (v['nome'], res, "Venda", vlr, it, get_now().isoformat()))
    ...
    with c_p:
        ...
        for _, v in vendedores[vendedores['status'] == 'Fora'].iterrows():
            ...
            m_pausa = v['motivo_pausa'] if v['motivo_pausa'] else "FORA"
...
# (Abas de Desempenho e Configuração seguem o mesmo padrão estável de antes...)
